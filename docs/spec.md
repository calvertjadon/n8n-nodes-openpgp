...
- **The incumbent does not get away with it either**: `@hapheus/n8n-nodes-pgp@1.5.0` fails the
  scanner at the provenance gate before linting even runs, and its published tarball reports six
  further errors under the same ruleset (`no-runtime-dependencies`, three `no-restricted-imports`,
  a forbidden `preinstall` script, an invalid homepage). `n8n-nodes-pgp@0.1.3` (2023) predates
