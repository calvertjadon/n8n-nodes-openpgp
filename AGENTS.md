...
Issues live in GitHub Issues on `calvertjadon/n8n-openpgp`, managed with the `gh` CLI. See `docs/agents/issue-tracker.md`.
...
│       └── ...
├── credentials/
│   └── OpenPgpPrivateKey.credentials.ts
├── package.json
└── ...
```
...
```json
{
  ...
  "n8n": {
    ...
    "strict": true,
    "credentials": [
        "dist/credentials/OpenPgpPrivateKey.credentials.js"
    ],
    "nodes": [
      "dist/nodes/OpenPgp/OpenPgp.node.js"
    ]
  }
}
```
